...
namespace Engine
{
	...
	void RenderPipeline::build(RenderGraph& graph, Scene* scene)
	{
		...
		ResourceHandle albedoBuffer = graph.Create("AlbedoAoBuffer", ResourceDesc::CreateColorTarget(backBufferDesc.width, backBufferDesc.height, DXGI_FORMAT_R16G16B16A16_FLOAT));
		...
		lightingPassParams.writes.push_back(backBuffer);
        // get shader
        ...
        graph.AddPass(
            ...
            PassType::Graphics,
            lightingPassParams,
            [positionBuffer, normalBuffer, depthBuffer, albedoBuffer, roughnessMetallicAoBuffer, backBuffer, lightingShader, &graph, scene, this](RenderCommandList& cmd)
            {
                // get resources
                auto* positionRes = graph.GetResource(positionBuffer);
                auto* normalRes = graph.GetResource(normalBuffer);
