...
namespace Game
{
    ...
    MeleeEnemyBehaviorTree::MeleeEnemyBehaviorTree(MeleeEnemyController* controller) : m_controller(controller)
    {
    }

    std::unique_ptr<BehaviorNode> MeleeEnemyBehaviorTree::SetupTree()
    {
        ...
        attackSeq.push_back(std::make_unique<BCheckTargetInMeleeAttackRange>(m_controller));
        attackSeq.push_back(std::make_unique<BMeleeAttack>(m_controller));
