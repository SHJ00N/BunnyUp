...
namespace Game
{
	...
	void GameScene::SceneEnter()
	{
        ...
        auto gameManagerObject = CreateGameObject<GameObject>("GameManager");
        auto gameManager = gameManagerObject->AddComponent<GameManager>();
        ...
        auto playerController = bunny->AddComponent<Game::PlayerController>();
        ...
        bunnySocket->Attach("Weapon", 21, staff, Vector3(0.0f), Vector3(0.0f, 0.0f, 2.5f), Vector3(1.0f));
        bunnySocket->Attach("Helmet", 36, earmuff, Vector3(0.0f), Vector3(1.7f, -1.45f, -3.0f), Vector3(1.0f));
        bunnySocket->Attach("Cape", 9, scarf, Vector3(3.0f, 0.0f, 0.0f), Vector3(4.75f, 0.0f, -1.5f), Vector3(1.0f));
        ...
        // UI
        // -------------------------------------------------------------------------------------------------------
        auto playerHealthTextObject = CreateGameObject<GameObject>("PlayerHealthText");
        playerHealthTextObject->transform.SetLocalPosition(Vector3(20.0f, 60.0f, 0.0f));

        auto playerHealthText = playerHealthTextObject->AddComponent<TextComponent>();
        playerHealthText->SetText("Player Health");
        playerHealthText->SetFont(ResourceManager::GetInstance().GetFont("Square"));
        playerHealthText->SetFontSize(64);
        playerHealthText->SetColor(Vector4(0.8f, 0.2f, 0.2f, 1.0f));
        gameManager->healthText = playerHealthText;

        auto killCountTextObject = CreateGameObject<GameObject>("KillCountText");
        killCountTextObject->transform.SetLocalPosition(Vector3(350.0f, 60.0f, 0.0f));

        auto killCountText = killCountTextObject->AddComponent<TextComponent>();
        killCountText->SetText("Kill Count");
        killCountText->SetFont(ResourceManager::GetInstance().GetFont("Square"));
        killCountText->SetFontSize(64);
        killCountText->SetColor(Vector4(0.1f, 0.1f, 0.1f, 1.0f));
        gameManager->killCountText = killCountText;

        auto gameOverTextObject = CreateGameObject<GameObject>("GameOverText");
        gameOverTextObject->transform.SetLocalPosition(Vector3(300.0f, 540.0f, 0.0f));
        gameOverTextObject->transform.SetLocalScale(Vector3(2.5f, 2.5f, 1.0f));

        gameOverTextObject->AddComponent<GameOverText>();

        auto gameOverText = gameOverTextObject->AddComponent<TextComponent>();
        gameOverText->SetText("Game Over");
        gameOverText->SetFont(ResourceManager::GetInstance().GetFont("Square"));
        gameOverText->SetFontSize(128);
        gameOverText->SetColor(Vector4(1.0f, 1.0f, 1.0f, 1.0f));
        gameManager->gameOverText = gameOverText;
        gameOverTextObject->Disable();

        auto restartTextObject = CreateGameObject<GameObject>("RestartText");
        restartTextObject->transform.SetLocalPosition(Vector3(200.0f, 800.0f, 0.0f));
        restartTextObject->transform.SetLocalScale(Vector3(2.0f, 2.0f, 1.0f));

        restartTextObject->AddComponent<StartText>();

        auto restartText = restartTextObject->AddComponent<TextComponent>();
        restartText->SetText("Press Enter is restart the game");
        restartText->SetFont(ResourceManager::GetInstance().GetFont("Square"));
        restartText->SetFontSize(64);
        restartText->SetColor(Vector4(0.8f, 0.8f, 0.8f, 1.0f));
        gameManager->restartText = restartText;
        restartTextObject->Disable();

        auto gameClearTextObject = CreateGameObject<GameObject>("GameClearText");
        gameClearTextObject->transform.SetLocalPosition(Vector3(225.0f, 540.0f, 0.0f));
        gameClearTextObject->transform.SetLocalScale(Vector3(2.5f, 2.5f, 1.0f));

        auto gameClearText = gameClearTextObject->AddComponent<TextComponent>();
        gameClearText->SetText("Game Clear!");
        gameClearText->SetFont(ResourceManager::GetInstance().GetFont("Square"));
        gameClearText->SetFontSize(128);
        gameClearText->SetColor(Vector4(1.0f, 1.0f, 1.0f, 1.0f));
        gameManager->gameClearText = gameClearText;
        gameClearTextObject->Disable();
    ...
    void GameScene::createTree(GameObject* parent, const std::string& name, const Vector3& pos, const Vector3& scale)
    {
    ...
    void GameScene::createGift(GameObject* parent, const std::string& name, int type, const Vector3& pos, const Vector3& rot, const Vector3& scale)
    {
